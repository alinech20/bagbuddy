...
export const useErrorHandler = () => {
  ...
  const createApiError = (error: any, userMessage?: string): IApiError => {
    ...
    const getBaseError = (): IApiError => ({
      ...
      technical: {
        message: `${error.data.statusCode} ${error.data.error}: ${error.data.message}` || 'Unknown error occurred',
    ...
    const populateError = (err: IApiError): IApiError => ({
      ...
      technical: {
        message: `${error.data.statusCode} ${error.data.error}: ${error.data.message}` || 'Request failed',
        originalError: error.error,
      },
    })
    ...
    const setExtraErrorData = (err: IApiError, status: number): IApiError => {
      switch (status) {
        ...
        case 422:
          ...
          err.user.message = userMessage || 'Please check your input and try again.'
          break
        case 401:
        ...
        case 404:
          ...
          err.user.message = userMessage || 'Resource not found. Please check the URL.'
          break
        default:
          if (status >= 500) {
            err.type = ERROR_TYPE.SERVER
            err.severity = ERROR_SEVERITY.CRITICAL
            err.user.message = userMessage || 'Server error. Please try again later.'
